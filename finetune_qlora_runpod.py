...
# %% [markdown]
# ## 0a — GPU pre-flight (fast P100 exit before expensive pip install)

# %%
# Check GPU capability FIRST — before installing anything.
# Kaggle randomly assigns P100 (sm_60) or T4 (sm_75).
# PyTorch 2.x ships NO sm_60 CUDA kernels → any CUDA op crashes with
# "no kernel image is available for execution on the device".
# Exit immediately on P100 so the retry wastes 2 min, not 20 min.
import torch, json, pathlib, sys

if torch.cuda.is_available():
    _sm = torch.cuda.get_device_capability(0)
    _sm_int = _sm[0] * 10 + _sm[1]
    print(f"GPU pre-flight: {torch.cuda.get_device_name(0)}  sm_{_sm_int}  "
...
# %% [markdown]
# ## 0b — Install dependencies (transformers from git for Gemma 4 support)
...
# ── Workaround: transformers 5.x dev PreTrainedAudioTokenizerBase NameError ──
# PreTrainedAudioTokenizerBase.__init_subclass__ calls get_type_hints(cls) which
# tries to evaluate "PeftConfigLike" from modeling_utils globals — only bound
# under TYPE_CHECKING, never at runtime.  Monkey-patch to swallow NameError.
...
# P100 (sm_60) exits in Cell 0a before reaching here, so we only run on T4+ (sm_75+).
USE_4BIT = torch.cuda.is_available()   # always True on T4/A100, False on CPU-only
if torch.cuda.is_available():
...
print("Device:", DEVICE, "| USE_4BIT:", USE_4BIT)

# %% [markdown]
# ## 2 — Load model (QLoRA 4-bit on T4+ | fp16 LoRA on P100)
...
def make_audio_response(gt_label: str) -> str:
    ...
    return json.dumps({
        ...
        "audio_analysis": {
            ...
            "respiratory_findings": {
                "cough_present":     False,          # not inferred from audio alone
...
if USE_4BIT:
    ...
    model_eval = AutoModelForImageTextToText.from_pretrained(
        GEMMA_MODEL_PATH, quantization_config=_eval_bnb,
...
for item in aud_val[:6]:
    ...
    msgs = [{"role": "system", "content": AUDIO_SYSTEM},
            {"role": "user", "content": [{"type": "audio"},
                                          {"type": "text", "text": "Analyze this respiratory sound."}]}]
    prompt = processor.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
    inp    = processor(text=prompt, audios=[(arr, AUDIO_SR)], return_tensors="pt").to(DEVICE)
    with torch.no_grad():
